...
public class LogTask implements Runnable {
	...
	public void run() {
		...
		if ((mLogType & LogTool.LOG_TYPE_2_LOGCAT) > 0) {
			Log.d(mTag, mMsg);
		}

		if ((mLogType & LogTool.LOG_TYPE_2_FILE) > 0) {
			log2File("ms:" + System.currentTimeMillis() + " [" + mTag + "]"
					+ mMsg);
		}

		if ((mLogType & LogTool.LOG_TYPE_2_NETWORK) > 0) {
	...
	private void log2File(String msg) {
		if (Utils.sdAvailible()) {
			...
			try {
				String systemInfo = "\n";
				File file = new File(LogTool.getLogFileName(mContext));
